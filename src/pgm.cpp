...
static std::string gMemName;
static __thread bool gIsGraphMaster = false;
static bi::managed_shared_memory *gGraphSharedMem = 0;
static struct pgm_graph* gGraphs = 0;
static bfs::path gGraphPath;
                ...
                pgm_edge* edge)
{
    ...
    {
        std::string fifoName(fifo_name(g, producer, consumer, edge));

        bfs::path fifoPath(gGraphPath);
        fifoPath /= fifoName;
...
{
    int ret = -1;
    bfs::path mqPath("/"); // msg queues always start at root
...
static int safePath(const bfs::path& dir)
{
    static const bfs::path allowed[] = {
        bfs::path("/dev/shm"),
        bfs::path("/home"),
        bfs::path("/tmp")
    };
    ...
    if(dirAbs.empty() || dirAbs == bfs::path("/"))
        return -1;
    ...
    {
        for(bfs::path::iterator compare(allowed[i].begin()), toCheck(dirAbs.begin());
...
static int prepare_dir(const bfs::path& graphDir)
{
    int ret = -1;
    ...
    if(boost::filesystem::equivalent(graphDir, bfs::current_path()))
    {
...
static std::string get_gMemName(const bfs::path& graphDir)
{
    ...
    ss<<std::hex<<hash<<"_"<<graphName;

    return ss.str();
}

static int prepare_graph_shared_mem(const bfs::path& graphDir)
{
    int ret = -1;

#ifdef PGM_SHARED
    std::string memName = get_gMemName(graphDir);
...
static int open_graph_shared_mem(const bfs::path& graphDir, const int timeout_s = 60)
{
    int ret = -1;
#ifdef PGM_SHARED
    int time = 0;
...
{
    int ret = -1;
    bfs::path graphDir(dir);

    if(graphDir.is_relative())
    {
        graphDir = bfs::current_path();
        graphDir /= dir;
    }
...
int pgm_destroy(void)
{
    ...
    if(gGraphSharedMem)
    {
        ...
        if(gIsGraphMaster)
            bi::shared_memory_object::remove(gMemName.c_str());
